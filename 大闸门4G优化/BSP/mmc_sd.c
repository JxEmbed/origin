...
//data:要写入的数据
//返回值:读到的数据
void SD_SPI_Delay(__IO uint32_t count)
{
	uint32_t i;
//	for(i=0;i<count;i++)
//	{
//		uint8_t uc =12;while(uc--);
//	}
}
u8 SD_SPI_ReadWriteByte(u8 ByteSend)
{
	...
	for (i = 0; i < 8; i ++)
	{
		SD_SPI_MOSI= ((ByteSend & (0x80 >> i))>0);
		SD_SPI_SCLK=0;
//		delay_us(2);
		SD_SPI_Delay(SD_Timeout);
		if (SD_SPI_MISO == 1)
		{
			ByteReceive |= (0x80 >> i);
		}
...
void SD_Write(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)   
{
	...
	secremain=512-secoff;//扇区剩余空间大小   
 	if(NumByteToWrite<=secremain)secremain=NumByteToWrite;//不大于512个字节
	while(1)
	{
		...
		SD_ReadDisk(SD_BUF,secpos,1);//写入整个扇区  
		SEGGER_RTT_SetTerminal(2); 
		...
		SEGGER_RTT_SetTerminal(0); 
		SD_WriteDisk(SD_BUF,secpos,1);//写入整个扇区  
