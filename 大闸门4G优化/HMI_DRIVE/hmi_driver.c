...
void BEGIN_CMD()
{
	xSemaphoreTake(LCD_semTx,RS485_xBlockTime);
	lcd_txptr=0;
    TX_8(0XEE);
...
void END_CMD()
{
    ...
    TX_16(crc16);                         //发送CRC16
    TX_32(0XFFFCFFFF);
	LCD_DMA_init(lcd_txptr);
}

#else//NO CRC16

#define SEND_DATA(P) SendChar(P)          //发送一个字节
#define BEGIN_CMD() TX_8(0XEE);xSemaphoreTake(LCD_semTx,RS485_xBlockTime)  //申请互斥信号量//帧头
#define END_CMD() TX_32(0XFFFCFFFF);xSemaphoreGive(LCD_semTx)       //帧尾
...
/*2024年5月17日11:13:02
EE B1 10 00 07 00 06 FF FC FF FF 
*/

void QRCode_Set(uint16 screen_id,uint16 control_id,char * txt_str)
{
	char * strptr=txt_str;
    BEGIN_CMD();
    TX_8(0xB1);
